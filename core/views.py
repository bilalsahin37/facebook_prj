...
def like_comment(request):
    ...
    if user in comment.likes.all():
        ...
        if comment.user != user:
            send_notification(
                comment.user, user, comment.post, comment, noti_comment_liked
            )
...
def reply_comment(request):
    ...
    if comment.user != user:
        send_notification(
            comment.user, user, comment.post, comment, noti_comment_replied
        )
...
def search_users(request):
    ...
    if query:
        users = (
            ...
            | User.objects.filter(full_name__icontains=query)
        )
...
def load_more_posts(request):
    all_posts = Post.objects.filter(active=True, visibility="Everyone").order_by(
        "-date"
    )
